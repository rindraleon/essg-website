...
function getToken(): string | undefined {
    ...
    if (typeof (globalThis as unknown as { window?: unknown }).window === "undefined") return undefined;
    ...
    (error) => {
        ...
        if (status === 401) {
            ...
            if (typeof (globalThis as unknown as { window?: unknown }).window !== "undefined") {
                ...
                (globalThis as unknown as { window: { location: { replace: (url: string) => void } } }).window.location.replace(loginUrl);
            }
        }
