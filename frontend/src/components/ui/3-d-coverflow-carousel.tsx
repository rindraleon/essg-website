...
function getCarouselCardStyle(offset: number, total: number): CoverFlowCardStyle {
  if (offset === 0) {
    return {
...
export function CoverFlowCarousel({
  ...
  className = '',
  onCtaClick,
}: Readonly<CoverFlowCarouselProps>) {
      ...
      <div className="relative w-full max-w-6xl mx-auto px-4 z-10 flex flex-col items-center">
        ...
        >
          {items.map((item, idx) => {
            ...
            const { transform, opacity, zIndex, filter, isCenter } = getCarouselCardStyle(
              offset,
              total
            );
              ...
              >
                ...
                {!isCenter && (
                  <button
                    ...
                    aria-label={`Voir ${item.titleLine1}`}
                    style={{
                      position: 'absolute',
                      inset: 0,
                      width: '100%',
