...
const Input = React.forwardRef<HTMLInputElement, React.ComponentProps<'input'>>(
  ({ className, type = 'text', ...props }, ref) => (
    <input
      ...
      type={type}
      className={cn(
        'flex h-11 w-full rounded-xl border border-ink-200 bg-white px-3.5 py-2 text-small text-ink-900 shadow-[0_1px_0_rgba(27,31,34,0.02)]',
        'placeholder:text-ink-400 placeholder:text-small',
        'transition-[border-color,box-shadow,background-color] duration-(--duration-fast) ease-(--ease-out-soft)',
