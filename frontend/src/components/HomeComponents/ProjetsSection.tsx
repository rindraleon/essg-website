...
const ProjetsSection: React.FC<FeaturedProjetsSectionProps> = (
    props: Readonly<FeaturedProjetsSectionProps>,
) => {
    const {
    ...
    return (
        ...
        >
            <ScrollableCardGrid className="mt-2 w-full">
                {projets.map((projet) => {
                    const imageUrl = projet.image
                        ? getImageUrl(projet.image)
                        : FALLBACK_IMAGE;
