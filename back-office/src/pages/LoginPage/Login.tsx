...
const styles = `
  ...
  /* Halos colorés en mouvement lent, pour donner de la profondeur au fond. */
  ...
  /* Défilement du réseau de courbes de niveau (clin d'œil à la géomatique). */
  @keyframes login-drift {
    from { background-position: 0 0; }
    to   { background-position: 220px 140px; }
  }
  /* Ken Burns : zoom et panoramique très lents sur la photographie de fond.
     Amplitude volontairement faible (4 %) pour rester discret et éviter
     que les bords ne se découvrent. */
  @keyframes login-kenburns {
    0%   { transform: scale(1.04) translate3d(0, 0, 0); }
    50%  { transform: scale(1.09) translate3d(-1.2%, -0.8%, 0); }
    100% { transform: scale(1.04) translate3d(0, 0, 0); }
  }
  /* Fondu d'arrivée de l'image, pour éviter l'apparition brutale. */
  @keyframes login-bg-in {
    from { opacity: 0; }
    to   { opacity: 1; }
  }
  /* Reflet qui traverse la carte à l'apparition. */
  ...
  /* Cascade : chaque bloc entre légèrement après le précédent. */
  ...
  /* Le champ actif se soulève très légèrement : retour visuel immédiat. */
...
/** Courbes de niveau, inspirées du motif déjà utilisé sur le site public. */
const TOPO_PATTERN =
...
export default function Login() {
  ...
  /** Style commun aux deux champs : verre translucide sur fond sombre. */
      ...
      <div className="relative flex min-h-[100dvh] items-center justify-center overflow-hidden bg-brand-950 px-4 py-8 sm:px-6">
        ...
        <div aria-hidden className="pointer-events-none absolute inset-0">
          <div className="absolute inset-0 overflow-hidden">
            <picture>
              {/* Version légère servie aux petits écrans (100 Ko au lieu de 312) */}
          ...
          {/* Vignettage : concentre le regard sur la carte */}
        ...
        <div className="relative w-full max-w-md">
          <div className="login-rise relative overflow-hidden rounded-3xl border border-white/20 bg-brand-650/45 p-6 shadow-[0_24px_70px_-20px_rgba(0,0,0,0.55)] backdrop-blur-2xl sm:p-8">
            {/* Reflet traversant, purement décoratif */}
            <span
              aria-hidden
            ...
            <header className="login-fade login-s1 mb-7 text-center">
              {/* <div className="mb-4 flex justify-center">
                <div className="rounded-2xl border border-white/25 bg-white/95 px-4 py-3 shadow-[0_8px_24px_-8px_rgba(0,0,0,0.5)]">
                  <img
                    src={EssG}
                    alt="Logo ESSG"
