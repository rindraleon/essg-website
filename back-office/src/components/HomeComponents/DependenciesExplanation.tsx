...
const DESCRIPTIONS: Record<string, string> = {
  ...
  react: "Bibliothèque UI principale (React) — moteur de rendu des composants de l'application.",
  'react-dom':
    ...
    <section className="py-12 text-gray-900" aria-labelledby="deps-heading">
      <div className="max-w-4xl mx-auto px-6 lg:px-8">
        ...
        <p className="text-sm text-gray-600 mb-6">
          Ci‑dessous la liste des dépendances présentes dans le template et une courte explication
          de leur utilité.
        </p>
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0">
                  <div className="flex items-center gap-3">
                    <h3
                      id={`dep-${it.name}`}
                      className="text-sm font-semibold text-gray-900 break-word"
                    >
                      {it.name}
                    </h3>
                    <span className="text-xs text-gray-500 px-2 py-0.5 bg-white border rounded">
                      {it.version}
                    </span>
                  </div>
